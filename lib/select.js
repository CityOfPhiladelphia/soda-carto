...
module.exports = function (columns, opts) {
  ...
  if (columns === '*') {
    ...
    return columns.map(function (col) {
      if (col.expr.type === 'column_ref' && col.expr.column === opts.geomAlias) {
        // If explicitly selecting geometry field (by its alias), wrap it in a format converter
        ...
        if (col.expr.name === 'convex_hull') {
          col.expr = {
            type: 'raw',
            value: 'ST_ConvexHull(ST_Collect(the_geom))'
          }
        }
      }
      return col
